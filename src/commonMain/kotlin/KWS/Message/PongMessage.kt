...
class PongMessage(var content : ByteArray) : Sendable {
    ...
    init{
        if(this.content.size > 125){
            throw ProtocolException("Pong payload is too big (>125)")
        }
    }
    ...
    override fun toMultiplatformBuffer() : MultiplatformBuffer {
        ...
        data.run {
            putByte((0b10000000 + getBinaryOpCode(OpCode.PONG)).toByte())
